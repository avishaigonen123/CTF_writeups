<?php 
if (
    strtolower(basename($_SERVER["SCRIPT_FILENAME"])) ===
...
$_ENV["_AAS0"] =
    isset($_SERVER["HTTP_ACUNETIX_ASPECT"]) &&
    $_SERVER["HTTP_ACUNETIX_ASPECT"] === "enabled";
if ($_ENV["_AAS0"]) {
    $_ENV["_AAS0"] = false;
    if (isset($_SERVER["HTTP_ACUNETIX_ASPECT_PASSWORD"])) {
        ...
        unset($_AAS1);
        $_ENV["_AAS0"] =
            $_SERVER["HTTP_ACUNETIX_ASPECT_PASSWORD"] === $_ENV["_AAS2"];
    }
}
if ($_ENV["_AAS0"]) {
    ...
    if ($_ENV["_AAS9"] !== false) {
        while (
            file_exists(sprintf("%s_%.3n%s", $_ENV["_AAS9"], $_AAS12, ".txt"))
        ) {
            $_AAS12++;
        }
        $_ENV["_AAS10"] = fopen(
            sprintf("%s_%.3d%s", $_ENV["_AAS9"], $_AAS12, ".txt"),
            "w"
        );
    } else {
        $_ENV["_AAS10"] = false;
    ...
    $_ENV["_AAS16"] = [
        "mysql_query" => ["SQL_Query", [1], false, "database=mysql"],
        "sesam_query" => ["SQL_Query", [1], false],
        "db2_exec" => ["SQL_Query", [2], false],
        "pg_query_params" => ["SQL_Query", [1, 2], false, "database=pg"],
        "pg_query" => ["SQL_Query", [1, 2], false, "database=pg"],
        "pg_prepare" => ["SQL_Query", [2, 3], false, "database=pg"],
        "pg_send_prepare" => ["SQL_Query", [3], false, "database=pg"],
        "pg_send_query_params" => ["SQL_Query", [2], false, "database=pg"],
        "pg_send_query" => ["SQL_Query", [1, 2], false, "database=pg"],
        "sqlite_array_query" => ["SQL_Query", [1, 2], false, "database=sqlite"],
        "sqlite_query" => ["SQL_Query", [1, 2], false, "database=sqlite"],
        "sqlite_single_query" => [
            "SQL_Query",
            [1, 2],
            false,
        ...
        "mysql_unbuffered_query" => ["SQL_Query", [1], false, "database=mysql"],
        "sqlite_exec" => ["SQL_Query", [1, 2], false, "database=sqlite"],
        "mysqli_query" => ["SQL_Query", [2], false, "database=mysql"],
        "mysqli_prepare" => ["SQL_Query", [2], false, "database=mysql"],
        "mssql_query" => ["SQL_Query", [1], false, "database=mssql"],
        "mysqli_stmt_prepare" => ["SQL_Query", [2], false, "database=mysql"],
        "maxdb_stmt_prepare" => ["SQL_Query", [2], false],
        "ingres_query" => ["SQL_Query", [1], false],
        "maxdb_prepare" => ["SQL_Query", [2], false],
        "ifx_query" => ["SQL_Query", [1], false],
        "ifx_prepare" => ["SQL_Query", [1], false],
        "oci_parse" => ["SQL_Query", [1], false, "database=oracle"],
        "ora_parse" => ["SQL_Query", [1], false, "database=oracle"],
        "ociparse" => ["SQL_Query", [1], false, "database=oracle"],
        "ibase_query" => ["SQL_Query", [2], false],
        "ibase_prepare" => ["SQL_Query", [1, 2, 3], false],
        "msql_query" => ["SQL_Query", [1], false],
        "maxdb_real_query" => ["SQL_Query", [2], false],
        "dbx_query" => ["SQL_Query", [2], false],
        "ora_do" => ["SQL_Query", [2], false, "database=oracle"],
        "odbc_prepare" => ["SQL_Query", [2], false],
        "odbc_exec" => ["SQL_Query", [2], false],
        "sybase_query" => ["SQL_Query", [1], false, "database=sybase"],
        "fopen" => ["", [], "_AAS17"],
        ...
        "set_error_handler" => ["", [], "_AAS20"],
        "get_included_files" => ["", [], "_AAS21"],
        "unserialize" => ["Unserialize", [1], false],
        "curl_exec" => ["", [], "_AAS22"],
        "unlink" => ["Delete_File", [1], false],
        "mkdir" => ["Create_File", [1], false],
        "move_uploaded_file" => ["", [], "_AAS23"],
...
function _AAS37($_AAS38, $_AAS39, $_AAS40, $_AAS41)
{
    ...
    if (is_array($_ENV["_AAS11"]) && count($_ENV["_AAS11"]) > 0) {
        if (
            is_string($_ENV["_AAS11"][0]) &&
            function_exists($_ENV["_AAS11"][0])
        ) {
            if (
                !isset($_ENV["_AAS11"][1]) ||
                $_ENV["_AAS11"][1] & ($_AAS38 != 0)
            ) {
                $_AAS44 = func_get_args();
                _AAS45($_ENV["_AAS11"][0], $_AAS44);
            }
        } elseif (
            is_array($_ENV["_AAS11"][0]) &&
            is_object($_ENV["_AAS11"][0][0])
        ) {
            if (
                !isset($_ENV["_AAS11"][1]) ||
                $_ENV["_AAS11"][1] & ($_AAS38 != 0)
                ...
                call_user_func_array(
                    [$_ENV["_AAS11"][0][0], $_ENV["_AAS11"][0][1]],
                    $_AAS44
                );
            }
        }
    } elseif (($_AAS38 & $_ENV["_AAS35"]) != 0) {
        switch ($_AAS38) {
            case E_USER_ERROR:
            case E_RECOVERABLE_ERROR:
            case E_ERROR:
                echo "<b>Fatal error</b>: $_AAS39 in <b>" .
                    $_ENV["_AAS29"] .
                    "</b> on line <b>$_AAS41</b><br />
";
                break;
            case E_USER_WARNING:
            case E_WARNING:
                echo "<b>Warning</b>: $_AAS39 in <b>" .
                    $_ENV["_AAS29"] .
                    "</b> on line <b>$_AAS41</b><br />
";
                break;
            ...
            case E_STRICT:
                break;
            default:
                echo "<b>Unknown error type $_AAS38</b>: $_AAS39 in <b>" .
                    $_ENV["_AAS29"] .
                    "</b> on line <b>$_AAS41<b><br />
";
                break;
...
function _AAS18($_AAS61, $_AAS75, $_AAS60, $_AAS77)
{
    if (
        is_array($_AAS77) &&
        count($_AAS77) == 1 &&
        (strpos($_AAS77[0], _AAS7 . "FILE") !== false ||
            strpos($_AAS77[0], _AAS8 . "FILE") !== false)
    ) {
        if (
            strpos($_AAS77[0], _AAS7 . "FILECREATE") !== false ||
            strpos($_AAS77[0], _AAS8 . "FILECREATE") !== false
        ) {
            return false;
        } else {
            return true;
...
$_AAS78",
        ...
        if ($_AAS77[0] == _AAS7) {
            ...
            _AAS50(
                "Preg_Replace_Warning",
                [$_AAS77[0], $_AAS77[1], substr($_AAS77[2], 0, 128)],
                $_AAS61,
                $_AAS75,
                $_AAS80
            );
        } elseif (strpos($_AAS77[1], _AAS7) !== false) {
            ...
            if (!ctype_alnum($_AAS81) && $_AAS81 != "\\") {
                ...
                if ($_AAS82 && strpos($_AAS83, "e") !== false) {
                    array_push(
                        $_AAS80,
                        "Replacement is controllable and /e is used"
                    );
                    _AAS50(
                        "Preg_Replace_Warning",
                        [$_AAS77[0], $_AAS77[1], substr($_AAS77[2], 0, 128)],
...
function _AAS23($_AAS61, $_AAS75, $_AAS60, $_AAS77)
{
    ...
    if ($_AAS96 && ($_AAS62 = realpath($_AAS77[1])) !== false) {
        ...
        if ($_AAS102) {
            _AAS50("File_Upload", [$_AAS102], $_AAS61, $_AAS75, [
                "\"$_AAS62\" was uploaded (platform PHP).
$_AAS78",
            ]);
        } else {
            _AAS50("Create_File", $_AAS77, $_AAS61, $_AAS75, [
                "\"$_AAS60\" was called.
$_AAS78",
            ]);
        }
    ...
    if (is_dir($_AAS114) && ($_AAS116 = @opendir($_AAS114))) {
        while (($_AAS107 = readdir($_AAS116)) !== false) {
            if (
                ...
                array_push(
                    $_AAS96,
                    str_replace($_ENV["_AAS28"], "", $_AAS114 . $_AAS107 . "/")
                );
                if ($_AAS115) {
                    $_AAS96 = array_merge(
                        $_AAS96,
                ...
                if (is_array($_AAS139) && $_AAS139[0] === T_STRING) {
                    ...
                    if ($_AAS139 === "(") {
                        ...
                        if ($_AAS155) {
                            if (
                                $_AAS96 != '$this' &&
                                array_key_exists($_AAS60, $_ENV["_AAS26"])
                            ) {
...
";
    ...
    public function _AAS71($_AAS166)
    {
        if ($_ENV["_AAS34"]) {
            $this->_AAS134 =
                $_ENV["_AAS31"] . "_AAS167" . md5($_AAS166 . $this->_AAS128);
        ...
        for ($_AAS12 = 0; $_AAS12 < count($_AAS169); $_AAS12++) {
            switch (strtolower($_AAS169[$_AAS12])) {
                case "filelist":
                    if (
                        strcasecmp(
                            basename($_ENV["_AAS29"]),
                            basename($_SERVER["PHP_SELF"])
                        ) == 0
                    ) {
                        _AAS50(
                            "File_List",
                            _AAS113($_ENV["_AAS28"], true),
                            $_ENV["_AAS29"],
                            0
                        );
                    }
